...
int feature_extraction_pipeline(int argc, char** argv) {
  ...
  for (i = 0; i < img_paths.size(); ++i) {
    ...
    for (scale = max_scale; scale >= min_scale; scale *= SCALE_STEP) {
      ...
      for (int yoff = 0; yoff < cv_img_scaled.rows - net_height + 1; ++yoff) {
        for (int xoff = 0; xoff < cv_img_scaled.cols - net_width + 1; ++xoff) {
          ...
          if ((cnt == batch_size) || (islast)) {
            ...
            for (int bi = 0; bi < cnt; ++bi) {
              for (int di = 0; di < argmax_dim; ++ di) {

                int pred = argmax_data[bi * argmax_dim + di];
                if (pred == 1) {
                  Dtype prob = argmax_data[bi * argmax_dim + di + 1];
