...
int feature_extraction_pipeline(int argc, char** argv) {
    ...
    if (argc < num_required_args) {
      ...
      "  pretrained_net_param  images_list_file extract_feature_blob_name save_feature_root_path"
      "  [CPU/GPU] [DEVICE_ID=0]\n"
    ...
    ifstream ifs(argv[3]);
    if(!ifs) {
      LOG(ERROR) << "Error opening file: " << string(argv[3]);
    } else {
      LOG(INFO) << "Image List file " << string(argv[3]) << " opened.";
    }
    ...
    // save dir
    string save_dir = argv[5];
    if (save_dir[save_dir.length() - 1] != '/') {
      save_dir += "/";
    }
    int ret = create_directory(argv[5]);
    if(0 != ret) {
