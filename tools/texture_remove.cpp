...
int feature_extraction_pipeline(int argc, char** argv) {
    ...
    int cnt = 0;
    int feat_index = 1;
    IplImage * tempImg = cvCreateImage(cvSize(net_width, net_height),IPL_DEPTH_8U, 1);
        ...
        for (int n = 0; n < feature_path.size(); ++n) {
          ...
          for(int i = 0; i < net_height; i++) {
            for (int j = 0; j < net_width; j++) {
              ((Dtype*)(tempImg->imageData + i*tempImg->widthStep))[j] = feature_blob_data[i*net_width+j];
              cout << feature_blob_data[i*net_width+j];
            }
          }
          Mat_<BYTE> imgGray = cvarrToMat(tempImg, true);
          imwrite(feature_path[n].c_str(), imgGray);

          std::cout << feature_path[n] << endl;
          std::string str;
          std::cin >> str;
