...
void PairDataLayer<Dtype>::load_pairs(string fn)
{
  ...
  ifs >> num_pairs;
  CHECK(num_pairs > 0);
  pairs_.reserve(num_pairs * 2);
  for (int i = 0; i < num_pairs; ++i) {
    if (!(ifs >> idx1 >> idx2)) {
      LOG(FATAL) << "Error reading pairs file: " << fn;
    }
    pairs_.push_back(idx1);
...
void PairDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  case PairDataParameter_DB_LEVELDB:
    ...
    LOG(INFO) << "Opening leveldb " << this->layer_param_.pair_data_param().source();
    leveldb::Status status = leveldb::DB::Open(
        options, this->layer_param_.pair_data_param().source(), &db_temp);
    CHECK(status.ok()) << "Failed to open leveldb "
                       << this->layer_param_.pair_data_param().source() << std::endl
                       << status.ToString();
    db_.reset(db_temp);
  ...
  case PairDataParameter_DB_LMDB:
    ...
    LOG(INFO) << "Opening lmdb " << this->layer_param_.pair_data_param().source();
  ...
  if (this->layer_param_.pair_data_param().rand_skip()) {
    unsigned int skip = caffe_rng_rand() %
                        this->layer_param_.pair_data_param().rand_skip();
  ...
  */
  ...
  if (crop_size > 0) {
    ...
    (*top)[2]->Reshape(this->layer_param_.pair_data_param().batch_size(),
                       datum.channels(), crop_size, crop_size);
    this->prefetch_data_.Reshape(this->layer_param_.pair_data_param().batch_size(),
        datum.channels(), crop_size, crop_size);
    this->prefetch_data2_.Reshape(this->layer_param_.pair_data_param().batch_size(),
        datum.channels(), crop_size, crop_size);
  } else {
    (*top)[0]->Reshape(
        this->layer_param_.pair_data_param().batch_size(), datum.channels(),
  ...
  this->prefetch_label_.Reshape(this->layer_param_.pair_data_param().batch_size(),
      1, 1, 1);
  this->prefetch_label2_.Reshape(this->layer_param_.pair_data_param().batch_size(),
      1, 1, 1);    
...
template <typename Dtype>
void PairDataLayer<Dtype>::Forward_cpu(
...
#ifdef DEBUG_WUHAO
  /*
  LOG(INFO) << "in PairDataLayer::Forward_cpu ";
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(PairDataLayer, Forward);
#endif
